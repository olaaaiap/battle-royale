...
void Partida::imprimirMenuInicial() {
	cout << R"(
...
int Partida::imprimirRequestPersonaje(string nombre, int ataque, int vida, int max) {
	int x;
...
int Partida::imprimirRequestEquipamiento(const vector<int>& listaUsados) {
	int x;
...
void Partida::imprimirMatriz(std::string matriz[25][40]) {
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 40; j++) {
			std::cout << matriz[i][j];
		}
...
void Partida::imprimirTotales(int& cantidadGuerreros, int& cantidadMagos, int& cantidadOgros,
...
void Partida::imprimirContadorRonda(int& ronda) {
	cout << " ---------------------------\n";
	cout << "          Ronda " << ronda << "         \n";
	cout << " ---------------------------\n";
}
...
void Partida::comprobarAtaque(string matriz[25][40], vector<Personaje>& personajes, int matrizDeIds[25][40], int& cantidadGuerreros, int& cantidadMagos, int& cantidadOgros,
	int& cantidadArqueras, int& cantidadDragones, int& cantidadVampiros) {
...
void Partida::cerrarArea(string matriz[25][40], vector<Personaje>& personajes,
...
void Partida::configurarPartida() {
	srand(time(NULL));
		...
		for (auto& p : personajes) {
			...
			if (p.GetAtacando() == false) {
				if (p.GetIrAlCentro()) {
					...
					p.MoverseHacia(centroX, centroY, matriz, matrizDeIds, p.GetId()); 
				}
				else {
